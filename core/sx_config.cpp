...
void config::parseServersData()
{
    ...
    __unused bool isDefaultParsedBefore = false;
    short lineNumber = 1;
    /*  Opening The File    */
    ...
    for (; getline(_configFile, line); )
    {
        ...
        if (!trimmedLine.empty() && trimmedLine == "server")
        {
            ...
            for (; getline(_configFile, line); )
            {
                ...
                {
                    if (tokens.size() > MAX_METHOD_SIZE)
                    ...
                    {   
                        ...
                        if (tokens[i] == "GET")
                            ...
                            currentConfig.setServerAllowedMethod(OPTIONS);
                        else if (tokens[i] == "HEAD")
                            currentConfig.setServerAllowedMethod(HEAD);
                        else if (tokens[i] == "PATCH")
                            currentConfig.setServerAllowedMethod(PATCH);
    ...
    else if (tokens[0] == "allowed_methods" && !currentLocationConfig.getAllowedMethods().size())
    {
        ...
        for (size_t i = 1; i < tokens.size(); i++)
        {
            if (tokens[i] == "GET")
                ...
                currentLocationConfig.setAllowedMethod(OPTIONS);
            else if (tokens[i] == "HEAD")
                currentLocationConfig.setAllowedMethod(HEAD);
            else if (tokens[i] == "PATCH")
                currentLocationConfig.setAllowedMethod(PATCH);
            else if (tokens[i] == "TRACE")
...
bool config::_parseAllowedMethods(std::vector<std::string> &tokens)
{
    ...
    knownAllowedMethods.insert("PUT");
    ...
    if (!valid || knownAllowedMethods.size() == MAX_METHOD_SIZE)
        return false;
    return true;
}
