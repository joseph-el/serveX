...
const char *methods[] = { "PUT", "GET", "HEAD", "POST", "PATCH", "TRACE" , "DELETE", "CONNECT", "OPTIONS", "UNKNOWN" };
...
void request::parseHeader(string target) {
    ...
    if (target == CRLF or target == LF) {
        ...
        short _st = _method & (GET | DELETE | OPTIONS | HEAD | TRACE) ? REQUEST_PARSE_DONE : REQUEST_BODY;
